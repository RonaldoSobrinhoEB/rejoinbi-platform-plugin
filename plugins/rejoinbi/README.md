...
Upload commands never filter project files by name or type: `.env`, private keys, token/password/credential files, archives, caches, and hidden folders are all uploaded when they belong to the project the user selected. The only automatic rejections are real path-traversal attempts and resumable-upload control artifacts, and the retired `--allow-sensitive-files` flag is accepted but has no effect (it is kept only for command-line compatibility). Plugin self-export (`export-package`) still keeps local session folders out of shareable zips, which is packaging hygiene for other people, not an upload limit. Raw API access requires the endpoint-derived `--operation-scope` and exact `--confirm-api-path`; `api-send` also requires `--yes`, while mapped identity endpoints additionally require `--identity-scope`.
