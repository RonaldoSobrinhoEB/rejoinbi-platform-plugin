...
python "$HOME\plugins\rejoinbi-platform\scripts\rejoinbi.py" --tenant subdomain.rejoinbi.com.br email schedule-manifests --page-id producao-geral
python "$HOME\plugins\rejoinbi-platform\scripts\rejoinbi.py" --tenant subdomain.rejoinbi.com.br email create-group --data-file C:\path\email-group.json --schedule-file C:\path\projeto\rejoinbi-schedule.json --yes
python "$HOME\plugins\rejoinbi-platform\scripts\rejoinbi.py" --tenant subdomain.rejoinbi.com.br whatsapp create-group --data-file C:\path\whatsapp-group.json --schedule-file C:\path\projeto\rejoinbi-schedule.json --yes
python "$HOME\plugins\rejoinbi-platform\scripts\rejoinbi.py" --tenant subdomain.rejoinbi.com.br email refresh-complete --page-id producao-geral --refresh-id producao-geral-08h --yes
...
Project-owned delivery schedules use `rejoinbi-schedule.json` at the project root or JSON files inside `schedules/`.
The manifest identifies the page, time, days, refresh-trigger mode, and delivery content. When a user selects the
manifest while creating an e-mail or WhatsApp group, the platform creates the runtime schedule and stores the
manifest id/path beside it. `after_update` opens the configured time window, waits for the page project revision to
change and become ready, then dispatches once for that group. The JSON example is available at
`examples/rejoinbi-schedule.json`.

If the project performs a refresh that does not change its published files, the refresh job can call
`email refresh-complete` or `whatsapp refresh-complete` after the update. That bridge updates the selected page
revision, wakes both channel schedulers through their existing five-second polling loop, and keeps the manifest id
