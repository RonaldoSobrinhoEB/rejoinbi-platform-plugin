...
- **Hybrid access (decide once, fallback fast)**: when the client application runs on the SAME server as the platform, it may open the managed SQLite file directly in read-only mode — `sqlite3.connect(r"file:<local_file_path>?mode=ro", uri=True)` — zero HTTP, zero lock contention (WAL readers never block the platform writer). `GET /plataforma/api/managed-databases/external/<id>/local-path` (Bearer) returns `local_file_path`, `local_open_uri` and `same_server`; the admin page (Integração tab, "Modo híbrido") shows the same path. Decision rule for generated apps: 1) check `os.path.exists(local_file_path)` ONCE and cache the decision; 2) exists -> open local `mode=ro`; 3) missing -> use the HTTPS API; 4) any local failure (missing file, error) -> switch to HTTPS and stop retrying local (circuit breaker, 60s). Writes ALWAYS go through the HTTPS API (the platform process owns the WAL); local access is read-only (`mode=ro`) regardless of token scope.
