...
- "carga em lote", "RPA alto volume", "envio em massa o banco", "55 mil linhas", "exportar X linhas": high-volume writes to a managed database must use the atomic bulk/transactional endpoints (`bulk_insert` / `statements`) with a keep-alive session, never one request per row. Consult `docs/managed-database-external-api.md`, respect the real limits from `/limits` (bulk_max_rows 50000 / teto 200000), and give the RPA its own `database_id` for isolation.
...
- When the connected project (or a remote RPA/agent) writes many rows, use the HTTPS API's atomic `bulk_insert` and transactional `statements` modes with a keep-alive session, and read the real limits from `/limits` before splitting lots. Never instruct a high-volume client to send one HTTP request per row. See `docs/managed-database-external-api.md` for the exact contract, token scopes, retry/backoff, and a ready Python client template.
...
- External managed database API (high volume): `POST /plataforma/api/managed-databases/external/<database_id>/query` (Bearer `rjdb_...` token) accepts legacy `{sql, params}`, atomic `{bulk_insert: {...}}`, and transactional `{statements: [...]}` payloads; `GET /plataforma/api/managed-databases/external/<database_id>/limits` returns the real limits. Read `docs/managed-database-external-api.md` before building or debugging a high-volume client.
